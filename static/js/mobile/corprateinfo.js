$(function () {
    ...
    $('#data-add').on('click', function (e) {
        layer.open({
            ...
            btn: ['确认', '取消'],
            yes: function (index) {
                $.post(path + '/corprateinfo/add.json', $('#form-edit').serialize(), function (result) {
                    if (result['success']) {
                        $('#data-query').click();
                    }
                    layer.close(index);
        ...
        if (checkedInputs.length > 0) {
            layer.alert('确定删除全部选中的数据吗?', {
                icon: 2, btn: ['确认', '取消'], yes: function (index) {
                    ...
                    form.push('</form>');
                    $.post(path + '/corprateinfo/delete.json', $(form.join('')).serialize(), function (result) {
                        if (result['success']) {
                            $('#data-query').click();
                        }
                    });
    ...
    $('#data-update').on('click', function (e) {
        layer.open({
            ...
            content: $('#modal-edit'),
            success: function (dom, index) {
                $.post(path + '/corprateinfo/get.json', {
            ...
            btn: ['确认', '取消'],
            yes: function (index) {
                $.post(path + '/corprateinfo/update.json', $('#form-edit').serialize(), function (result) {
                    if (result['success']) {
                        layer.close(index);
                        $('#data-query').click();
                    }
    ...
    $('#data-delete-all').on('click', function (e) {
        layer.alert('确定删除吗?', {
            icon: 2, btn: ['确认', '取消'], yes: function (index) {
                $.post(path + '/corprateinfo/delete.json', {
