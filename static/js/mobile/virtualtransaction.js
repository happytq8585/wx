$(function () {
    ...
    $('#data-add').on('click', function (e) {
        layer.open({
            ...
            btn: ['确认', '取消'],
            yes: function (index) {
                $.post(path + '/virtualtransaction/add.json', $('#form-edit').serialize(), function (result) {
                    if (result['success']) {
                        $('#data-query').click();
                    }
                    layer.close(index);
        ...
        if (checkedInputs.length > 0) {
            layer.alert('确定删除全部选中的数据吗?', {
                icon: 2, btn: ['确认', '取消'], yes: function (index) {
                    ...
                    form.push('</form>');
                    $.post(path + '/virtualtransaction/batchdelete.json', $(form.join('')).serialize(), function (result) {
                        if (result['success']) {
                            $('#data-query').click();
                        }
                    });
    ...
    $('.data-modify').on('click', function (e) {
        layer.open({
            ...
            content: $('#modal-edit'),
            success: function (dom, index) {
                $.post(path + '/virtualtransaction/get.json', {
            ...
            btn: ['确认', '取消'],
            yes: function (index) {
                $.post(path + '/virtualtransaction/update.json', $('#form-edit').serialize(), function (result) {
                    if (result['success']) {
                        layer.close(index);
                        $('#data-query').click();
                    }
    ...
    $('.data-stop').on('click', function (e) {
        layer.alert('确定停用吗?', {
            icon: 2, btn: ['确认', '取消'], yes: function (index) {
                $.post(path + '/virtualtransaction/stop.json', {
    ...
    $('.data-open').on('click', function (e) {
        layer.alert('确定启用吗?', {
            icon: 2, btn: ['确认', '取消'], yes: function (index) {
                $.post(path + '/virtualtransaction/open.json', {
                    ids: $(e.target).parents('tr').data('id')
                }, function (result) {
                    if (result['success']) {
                        layer.close(index);
    ...
    /*注销*/
    $('.data-distory').on('click', function (e) {
        layer.alert('确定注销吗?', {
            icon: 2, btn: ['确认', '取消'], yes: function (index) {
                $.post(path + '/virtualtransaction/distory.json', {
                    ids: $(e.target).parents('tr').data('id')
                }, function (result) {
                    if (result['success']) {
                        layer.close(index);
