$(function () {
    ...
    $('#data-add').on('click', function (e) {
        layer.open({
            ...
            yes: function (index) {
                ...
                $.post(path + '/reconciliationstatusinfo/add.json', formData, function (result) {
                    if (result['success']) {
                        $('#data-query').click();
                    }
                    layer.close(index);
            ...
            layer.alert('确定删除全部选中的数据吗?', {
                icon: 2, btn: ['确认', '取消'], yes: function (index) {
                    ...
                    $.each(ids, function (i, id) {
                        form.push('<input type="text" name="reconciliationstatusinfoIds" value="' + id + '">');
                    });
                    form.push('</form>');
                    $.post(path + '/reconciliationstatusinfo/batch_delete.json', $(form.join('')).serialize(), function (result) {
                        if (result['success']) {
                            $('#data-query').click();
                        }
                    });
    ...
    $('.data-modify').on('click', function (e) {
        layer.open({
            ...
            content: $('#modal-edit'),
            success: function (dom, index) {
                $.post(path + '/reconciliationstatusinfo/get.json', {
                    reconciliationstatusinfoId: $(e.target).parents('tr').data('id')
                }, function (result) {
                    if (result['success']) {
            ...
            btn: ['确认', '取消'],
            yes: function (index) {
                $.post(path + '/reconciliationstatusinfo/update.json', $('#form-edit').serialize(), function (result) {
                    if (result['success']) {
                        layer.close(index);
                        $('#data-query').click();
                    }
    ...
    $('.data-del').on('click', function (e) {
        layer.alert('确定删除吗?', {
            icon: 2, btn: ['确认', '取消'], yes: function (index) {
                $.post(path + '/reconciliationstatusinfo/delete.json', {
                    reconciliationstatusinfoId: $(e.target).parents('tr').data('id')
                }, function (result) {
                    if (result['success']) {
                        layer.close(index);
