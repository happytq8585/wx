$(function () {
    ...
    $('#data-add').on('click', function (e) {
        layer.open({
            ...
            btn: ['确认', '取消'],
            yes: function (index) {
                var submitData = $('#form-edit').serialize();
                submitData = submitData.replace("authno=&", "");
                $.post(path + '/terminalauthinfo/add.json', submitData, function (result) {
                    if (result['success']) {
                        $('#data-query').click();
                    }
                    layer.close(index);
            ...
            layer.alert('确定删除全部选中的数据吗?', {
                icon: 2, btn: ['确认', '取消'], yes: function (index) {
                    ...
                    $.each(ids, function (i, id) {
                        form.push('<input type="text" name="terminalAuthInfoIds" value="' + id + '">');
                    });
                    form.push('</form>');
                    $.post(path + '/terminalauthinfo/batch_delete.json', $(form.join('')).serialize(), function (result) {
                        if (result['success']) {
                            $('#data-query').click();
                        }
                    });
    ...
    $('.data-modify').on('click', function (e) {
        layer.open({
            ...
            content: $('#modal-edit'),
            success: function (dom, index) {
                $.post(path + '/terminalauthinfo/get.json', {
                    authno: $(e.target).parents('tr').data('id')
                }, function (result) {
                    if (result['success']) {
            ...
            yes: function (index) {
                var submitData = $('#form-edit').serialize();
                $.post(path + '/terminalauthinfo/update.json', submitData, function (result) {
                    if (result['success']) {
                        layer.close(index);
                        $('#data-query').click();
                    }
    ...
    $('.data-del').on('click', function (e) {
        layer.alert('确定删除吗?', {
            icon: 2, btn: ['确认', '取消'], yes: function (index) {
                $.post(path + '/terminalauthinfo/delete.json', {
                    terminalauthinfoId: $(e.target).parents('tr').data('id')
                }, function (result) {
                    if (result['success']) {
                        layer.close(index);
                        $('#data-query').click();
