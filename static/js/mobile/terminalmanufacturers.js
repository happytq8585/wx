$(function () {
    ...
    $('#data-add').on('click', function (e) {
        layer.open({
            ...
            btn: ['确认', '取消'],
            yes: function (index) {
                $.post(path + '/terminalmanufacturers/add.json', $('#form-edit').serialize(), function (result) {
                    if (result['success']) {
                        $('#data-query').click();
                    }
                    layer.close(index);
    ...
    $('#data-update').on('click',function(e){
        ...
        if(checkedInputs.length < 1){
            ...
            layer.open({
                ...
                content: $('#modal-edit'),
                success: function (dom, index) {
                    $.post(path + '/terminalmanufacturers/get.json', {
                ...
                btn: ['确认', '取消'],
                yes: function (index) {
                    $.post(path + '/terminalmanufacturers/update.json', $('#form-edit').serialize(), function (result) {
                        if (result['success']) {
                            layer.close(index);
                            $('#data-query').click();
                        }
            ...
            layer.alert('确定删除全部选中的数据吗?', {
                icon: 2, btn: ['确认', '取消'], yes: function (index) {
                    ...
                    $.each(ids, function (i, id) {
                        form.push('<input type="text" name="terminalmanufacturersIds" value="' + id + '">');
                });
            form.push('</form>');
                    $.post(path + '/terminalmanufacturers/delete_all.json', $(form.join('')).serialize(), function (result) {
                        if (result['success']) {
                            $('#data-query').click();
                        }
                    });
    ...
    $('.data-del').on('click', function (e) {
        layer.alert('确定删除吗?', {
            ...
            btn: ['确认', '取消'], yes: function (index) {
                $.post(path + '/terminalmanufacturers/delete.json', {
                    terminalmanufacturersId: $(e.target).parents('tr').data('id')
                }, function (result) {
                    if (result['success']) {
                        layer.close(index);
                        $('#data-query').click();
