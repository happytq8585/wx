$(function () {
    ...
    $('#data-add').on('click', function (e) {
        layer.open({
            ...
            yes: function (index) {
                ...
                formData = formData.replace("authno=&", "");
                $.post(path + '/reconciliationauthinfo/add.json', formData, function (result) {
                    if (result['success']) {
                        $('#data-query').click();
                    }
                    layer.close(index);
            ...
            layer.alert('确定删除全部选中的数据吗?', {
                icon: 2, btn: ['确认', '取消'], yes: function (index) {
                    ...
                    $.each(ids, function (i, id) {
                        form.push('<input type="text" name="reconciliationauthinfoIds" value="' + id + '">');
                    });
                    form.push('</form>');
                    $.post(path + '/reconciliationauthinfo/batch_delete.json', $(form.join('')).serialize(), function (result) {
                        if (result['success']) {
                            $('#data-query').click();
                        }
                    });
    ...
    $('.data-modify').on('click', function (e) {
        layer.open({
            ...
            content: $('#modal-edit'),
            success: function (dom, index) {
                $.post(path + '/reconciliationauthinfo/get.json', {
                    reconciliationauthinfoId: $(e.target).parents('tr').data('id')
                }, function (result) {
                    if (result['success']) {
            ...
            btn: ['确认', '取消'],
            yes: function (index) {
                $.post(path + '/reconciliationauthinfo/update.json', $('#form-edit').serialize(), function (result) {
                    if (result['success']) {
                        layer.close(index);
                        $('#data-query').click();
                    }
    ...
    $('.data-del').on('click', function (e) {
        layer.alert('确定删除吗?', {
            icon: 2, btn: ['确认', '取消'], yes: function (index) {
                $.post(path + '/reconciliationauthinfo/delete.json', {
                    reconciliationauthinfoId: $(e.target).parents('tr').data('id')
                }, function (result) {
                    if (result['success']) {
                        layer.close(index);
